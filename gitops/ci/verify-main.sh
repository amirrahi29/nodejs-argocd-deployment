...
# GitHub Actions sets VERIFY_* from the push branch; this repo defaults to staging for local runs.
set -euo pipefail

NS_APP="${VERIFY_NAMESPACE:-staging}"
P="${ARGO_APP_PREFIX:-chat-app}"
C="${HELM_CHART_NAME:-chat-app}"
DEPLOY="${VERIFY_DEPLOYMENT:-${P}-staging-${C}}"
ARGO_APP="${VERIFY_ARGO_APP:-${P}-staging}"
